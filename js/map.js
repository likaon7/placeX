	...
	initTimeSlider: function() {
		...
		fetchJSON('squares/0/0/stats_sorted.json').then(function(statsJSON) {
			...
			placeArmy.dateSlider.noUiSlider.on('update', function (values, handle) {
				...
				//TODo konwertuj na availableTimeslices w postaci ['min-max', 'min-max']
 ...
 */
	initContributorsMap: function(mode) {
		//if (!this.firstRun) placeArmy.reInit() //must reinitializa all map to remove all layers from CanvasFlowmap (bug?)
	...
	//TODO przycisk do wyswietlania allies
	//funkcja pobierajaca allies dla wszystkich wybranych placeów i łącząca pobrane jsony
	/**
	fetches json
	returns geoJSON
	 */
	...
	convertContribsGeneralToLayerGroup: function(id, placeID, placeCenter, ContributorsCoords, timespan = null) {
		...
		//TODO handle timespan
		//test
		//var timeslice = '1649111638315-1649114644314'
	...
	//TODO version for places mode (current v is for square mode only)
